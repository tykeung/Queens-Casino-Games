...
mines_menu::mines_menu(int window_width, float &balance, QWidget *parent) : QWidget(parent) {
    ...
    end_game->setStyleSheet("font-size: 11pt");
    ...
    mine_word->setFont(font);
    mine_word->setStyleSheet("font-size: 11pt");
        ...
        for(int j = 0; j < 5; j++) {
            QObject::connect(btn[i][j], &QPushButton::clicked, [start_game, bet, btn, end_game, mine, game_result, i, j, this](){
                if (minefield[i][j] == true) {
                    ...
                    for(int x = 0; x < 5; x++) {
                        for(int y = 0; y < 5; y++){
                            if (btn[x][y]->isEnabled()) {
                                if (minefield[x][y]) {
                                    btn[x][y]->setStyleSheet("background-color: #ffaeab;");
                                } else {
                                    btn[x][y]->setStyleSheet("background-color: #c9ffbd;");
                                }
                            }
                            btn[x][y]->setEnabled(false);
                        }
                    }
                    btn[i][j]->setStyleSheet("background-color: #a10600;");
                } else {
                    minefield[i][j] = true;
                    btn[i][j]->setEnabled(false);
                    ...
                    btn[i][j]->setStyleSheet("background-color: #28ad28;");
                }
        ...
        for(int i = 0; i < 5; i++) {
            for(int j = 0; j < 5; j++){
                if (btn[i][j]->isEnabled()) {
                    if (minefield[i][j]) {
                        btn[i][j]->setStyleSheet("background-color: #ffaeab;");
                    } else {
                        btn[i][j]->setStyleSheet("background-color: #c9ffbd;");
                    }
                }
                btn[i][j]->setEnabled(false);
            }
